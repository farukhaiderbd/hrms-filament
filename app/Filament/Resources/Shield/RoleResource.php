                        ...
                        Forms\Components\Card::make()
                            ->schema([
                                Forms\Components\TextInput::make('name')
                                    ...
                                    ->required()
                                    ->maxLength(255)
                                    ->afterStateUpdated(fn(\Filament\Forms\Set $set, $state): string => $set('name', Str::lower($state))),
                                Forms\Components\TextInput::make('guard_name')
                                    ->label(__('filament-shield::filament-shield.field.guard_name'))
                                    ->default(config('filament.auth.guard'))
                                    ->nullable()
                                    ->maxLength(255)
                                    ->afterStateUpdated(fn(\Filament\Forms\Set $set, $state): string => $set('guard_name', Str::lower($state))),
